...
while True:
    time.sleep(n)
    linAccTuple = imu.lin_acc()
   
    xLinAcc = linAccTuple[0]
    yLinAcc = linAccTuple[1]
    zLinAcc = linAccTuple[2]
    
    magnitude = math.sqrt(pow(xLinAcc, 2) + pow(yLinAcc, 2) + pow(zLinAcc, 2))
    print("Magnitude: ", magnitude)
    
    if magnitude > gravity_acc:
        print("Burningout...")
        loop += 1
        
        if loop == maxLoop:
            print("We have launched")
            break
        
        print((maxLoop - loop) * n, "  seconds away from completion")
        print(magnitude)
